...
def best_local_path(intersect_idx, roads, roads_o, remaining, radius=15):
  if radius == 0:
    return [[], 0, 0, 0]
  min_cost = sys.maxint
  max_distance = 0
  ...
  for road in nr:
    ...
    if float(distance) / (cost * (1 + current_malus)) > float(max_distance) / min_cost and remaining > cost:
      best_path = path
      min_cost = cost
